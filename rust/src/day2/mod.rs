        ...
        .fold(HashMap::new(), |mut game_with_mins, game_round| {
            {
                game_round.iter().for_each(|(color, n_cubes)| {
                    game_with_mins
                        .entry(color)
                        .and_modify(|current_val| *current_val = max(*current_val, n_cubes))
                        .or_insert(n_cubes);
                });
            }
            game_with_mins
...
pub fn calc_power_minimum_cubes(input: &str) -> Vec<u32> {
    ...
    game_rounds
        .iter()
        .map(calc_minimum_cubes)
        .map(|min_cubes| min_cubes.iter().fold(1, |power, n_cubes| power * n_cubes))
        .collect()
}
